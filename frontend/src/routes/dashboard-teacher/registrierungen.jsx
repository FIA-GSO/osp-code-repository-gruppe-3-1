        ...
        <div className="max-w-[1100px] p-8">
          ...
          <Card title="Alle Registrierungen">
            <div className="block overflow-x-auto md:table md:w-full">
              <select value={statusFilter} onChange={(e) => setStatusFilter(Number(e.target.value))} className="rounded-md border p-2 mr-4">
              ...
              <select value={eventFilter} onChange={(e) => setEventFilter(e.target.value)}className="rounded-md border p-2">
                    ...
                    <tr key={registration.id} className="bg-[#fafbfc]">
                      ...
                      <td className="p-3">
                        {registration?.status_id ===1 && (
                          ...
                          <button><StatusIcon type="pendingFull" /></button>
                          </span>
                        )}
                        {registration?.status_id === 2 && (
                          <span className="text-success-text">
                           <button><StatusIcon type="acceptedFull" /></button>
                          </span>
                        )}
                        {registration?.status_id === 3 && (
                          <span className="text-error-text">
                         <button><StatusIcon type="rejectedFull" /></button>
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-3">
                        {/* ✔ / ✖ NUR BEI OFFEN */}
                        {registration.status_id === 1 && (
                          <><button onClick={() => updateStatus(registration, 2)} className="mr-2 rounded-md bg-[#f1f3f6] px-3 py-1.5 hover:bg-[#e5e9ef]" title="Annehmen">
                                <StatusIcon type="accepted" />
                            </button>

                            <button onClick={() =>updateStatus(registration, 3)} className="mr-2 rounded-md bg-[#f1f3f6] px-3 py-1.5 hover:bg-[#e5e9ef]" title="Ablehnen">
